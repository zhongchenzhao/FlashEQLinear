...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., tranNum = 4):
        ...
        self.fc1 = FlashEQLinear(in_features, hidden_features, bias=True)
        self.act = act_layer()
        # self.fc2 = EQLinearInter(hidden_features, out_features, tranNum)
        self.fc2 = FlashEQLinear(hidden_features, out_features, bias=True)
        self.drop = EQDropout(drop, tranNum)
        # self.drop = E_Dropout(drop, tranNum)
    def forward(self, x):
        # sizeX = x.shape
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., tranNum = 4 ):
        ...
        self.qkv = FlashEQLinear(dim//tranNum, dim//tranNum*3, bias=True)

        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        # self.proj = EQLinearInter(dim//tranNum, dim//tranNum, tranNum)
        self.proj = FlashEQLinear(dim//tranNum, dim//tranNum, bias=True)
...
class SwinTransformer_EQ_PM_Conv(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, FlashEQLinear) or isinstance(m, EQLinearOutput):
            trunc_normal_(m.weights, std=.005)
            if isinstance(m, FlashEQLinear) and m.c is not None:
...
if __name__ == "__main__":
    ...
    def target_classes():
        ...
        return (FlashEQLinear, EQLinearOutput, EQPatchMergingConv)
    ...
    run_throughput_cli(
        ...
        tran_num=4,
        model_family="swin",
        title="Flash EQ-SwinTransformer",
