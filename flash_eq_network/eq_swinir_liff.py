"""Standalone, fully C4-equivariant SwinIR + LIIF-EQ.
...
The architecture follows Equivariant-ASISR with embed_dim=288:
  ...
  - LIIF-EQ: local ensemble, no feature unfold, cell decoding,
    74 -> 512(eq) -> 256(invariant) -> 256 -> 256 -> RGB.

Unlike the released swinir_eq.py, every LayerNorm affine parameter is tied
over the four orientation channels.
"""
...
class EQLinearInter(nn.Module):
    """C4 regular representation -> C4 regular representation."""
    ...
    def __init__(self, in_base, out_base, bias=True):
        ...
        self.out_base = out_base
        self.weight = nn.Parameter(
            torch.empty(out_base, 1, in_base, NUM_ROTATIONS)
        )
        if bias:
            self.bias = nn.Parameter(torch.empty(out_base))
        else:
            self.register_parameter("bias", None)
        index = torch.arange(NUM_ROTATIONS)
        shift_index = (
            index.view(1, NUM_ROTATIONS) - index.view(NUM_ROTATIONS, 1)
        ) % NUM_ROTATIONS
        self.register_buffer("shift_index", shift_index)
        self.reset_parameters()

    def reset_parameters(self):
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        if self.bias is not None:
            bound = 1.0 / math.sqrt(self.in_base * NUM_ROTATIONS)
            nn.init.uniform_(self.bias, -bound, bound)

    def forward(self, x):
        base_weight = self.weight.squeeze(1)
        expanded = base_weight[:, :, self.shift_index]
        expanded = expanded.permute(0, 2, 1, 3).reshape(
            self.out_base * NUM_ROTATIONS, self.in_base * NUM_ROTATIONS
        )
        bias = None if self.bias is None else self.bias.repeat_interleave(NUM_ROTATIONS)
        return F.linear(x, expanded, bias)
...
class EQLinearInput(nn.Module):
    ...
    def __init__(self, in_base, out_base, coord_scale=0.1):
        ...
        self.linear = EQLinearInter(in_base + 2, out_base)
        theta = -torch.arange(NUM_ROTATIONS, dtype=torch.float32)
...
class EQWindowAttention(nn.Module):
    ...
    def __init__(self, dim=EMBED_DIM, num_heads=6):
        ...
        self.qkv = EQLinearInter(dim // NUM_ROTATIONS, 3 * dim // NUM_ROTATIONS)
        self.proj = EQLinearInter(dim // NUM_ROTATIONS, dim // NUM_ROTATIONS)
...
class EQMLP(nn.Module):
    def __init__(self, dim=EMBED_DIM, ratio=2.0):
        ...
        self.fc1 = EQLinearInter(dim // NUM_ROTATIONS, hidden // NUM_ROTATIONS)
        self.fc2 = EQLinearInter(hidden // NUM_ROTATIONS, dim // NUM_ROTATIONS)
...
class EQSwinIR(nn.Module):
    ...
    def _init_transformer_weights(module):
        if isinstance(module, EQLinearInter):
            _trunc_normal_(module.weight, std=0.002)
...
class LIIFEQDecoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.eq_input = EQLinearInput(18, 128, coord_scale=0.1)
        self.eq_output = EQLinearOutput(128, 256)
        self.fc1 = nn.Linear(256, 256)
        self.fc2 = nn.Linear(256, 256)
        self.fc_rgb = nn.Linear(256, 3)

    def forward(self, x):
        x = F.relu(self.eq_input(x))
        x = self.eq_output(x)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
class EQSwinIRLIIF(nn.Module):
    """Complete EQ-SwinIR + LIIF-EQ model; instantiate with no arguments."""

    def __init__(self):
        super().__init__()
        self.encoder = EQSwinIR()
        self.decoder = LIIFEQDecoder()

    def query_rgb(self, feature, coord, cell):
        height, width = feature.shape[-2:]
        feature_coord = make_coord(
            (height, width), feature.device, feature.dtype
        ...
        for shift_y in (-1, 1):
            for shift_x in (-1, 1):
                ...
                query_feature = F.grid_sample(
                    feature, grid, mode="nearest", align_corners=False
                )[:, :, 0, :].permute(0, 2, 1)
                ...
                decoder_input = torch.cat(
                    (query_feature, cell_y, cell_x, relative_coord), dim=-1
                )
                batch, queries = coord.shape[:2]
                prediction = self.decoder(
                    decoder_input.reshape(batch * queries, -1)
                ).reshape(batch, queries, 3)
    ...
    def forward(self, image, coord, cell):
        return self.query_rgb(self.encoder(image), coord, cell)


class EQSwinIRLIIFThroughput(nn.Module):
...
if __name__ == "__main__":
    ...
    def build_model(model_name, image_size):
        ...
        return EQSwinIRLIIFThroughput(image_size=image_size, scale=2)


    run_throughput_cli(
        build_model=build_model,
        target_classes=(EQLinearInter, EQLinearOutput, nn.Linear),
        tran_num=NUM_ROTATIONS,
        model_family="eq-swinir-liif",
        title="EQ-SwinIR + LIIF-EQ",
