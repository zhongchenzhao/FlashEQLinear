...
def selective_scan_fn(
    ...
    available = {
        "oflex": WITH_SELECTIVESCAN_OFLEX,
        "core": WITH_SELECTIVESCAN_CORE,
        "mamba": WITH_SELECTIVESCAN_MAMBA,
    }
    if backend not in (None, "torch", *available):
        raise ValueError(f"Unknown selective-scan backend: {backend!r}")
    if u.is_cuda and backend in available and not available[backend]:
        raise ImportError(
            f"Selective-scan CUDA backend {backend!r} is unavailable. "
            "Install the matching VMamba/Mamba CUDA extension, or use "
            "backend=None for automatic selection or backend='torch'."
        )
    WITH_CUDA = any(available.values())
    fn = selective_scan_torch if backend == "torch" or not u.is_cuda or (not WITH_CUDA) else SelectiveScanCuda.apply
...
def print_jit_input_names(inputs):
    ...
    for value in inputs[:10]:
        print(value.debugName(), end=" ", flush=True)
    print("", flush=True)
