#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""FlashEQLinear-accelerated C4-equivariant Vision Transformer."""
...
class Attention(nn.Module):
    def __init__(self, dim: int, num_heads: int = 8, qkv_bias: bool = True,
        ...
        self.qkv = FlashEQLinear(dim // 4, dim * 3 // 4, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = FlashEQLinear(dim // 4, dim // 4, bias=True)
...
class FlashMlp(nn.Module):
    def __init__(self, dim: int, hidden_dim: int, drop: float = 0.):
        super().__init__()
        self.fc1 = FlashEQLinear(dim // 4, hidden_dim // 4, bias=True)
        self.act = nn.GELU()
        self.drop1 = nn.Dropout(drop)
        self.fc2 = FlashEQLinear(hidden_dim // 4, dim // 4, bias=True)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 4., qkv_bias: bool = True,
        ...
        self.mlp = FlashMlp(dim, int(dim * mlp_ratio), proj_drop)
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size: Union[int, Tuple[int, int]] = 224, patch_size: int = 16,
        ...
        self.proj = FConvPCA(patch_size, in_chans, embed_dim // 4, tran_num=4,
                             stride=patch_size, first_layer=True, bias=bias)
