...
python eq_vmamba_v2.py --scale tiny --precision fp32; 
python eq_vmamba_v2.py --scale small --precision fp32; 
python eq_vmamba_v2.py --scale base --precision fp32; 
python eq_vmamba_v2.py --scale large --precision fp32; 
python eq_vmamba_v2.py --scale huge --precision fp32;
"""
...
class EQ_linear_inter(nn.Module):
    def __init__(self, inNum, outNum, tranNum=4, bias=True, iniScale=1.0):
        ...
        # Match the cached Naive EQ path used by EQ-ViT/EQ-Swin.  The index
        # directly builds the block-circulant matrix without repeat + six cats.
        ...
        self._cached_parameter_version = -1
        self.reset_parameters()

    def _expand_weight(self):
        weight = self.weights.squeeze(1)
        rolled = weight[:, :, self.shift_idx]
        return rolled.permute(0, 2, 1, 3).reshape(
    ...
    def _weight_bias(self):
        # Caching a detached dense matrix is inference-only.  Eval with gradients
        # still uses the differentiable expansion, preserving normal PyTorch
        # semantics for validation-time gradient analysis.
        use_cache = not self.training and not torch.is_grad_enabled()
        if not use_cache:
            return self._expand_weight(), self._expand_bias()

        version = int(self.weights._version)
        if self.c is not None:
            version = (version << 32) ^ int(self.c._version)
        if self._cached_weight is None or version != self._cached_parameter_version:
            self._cached_weight = self._expand_weight().detach()
            self._cached_bias = (
                self._expand_bias().detach() if self.use_bias else None
            )
            self._cached_parameter_version = version
        return self._cached_weight, self._cached_bias

    # Backward-compatible helper name used by older local tests.
    def _expanded_weight_bias(self):
        return self._expand_weight(), self._expand_bias()

    def forward(self, input):
        """Apply EQ-Linear to packed last-axis or packed NCHW input."""
        expected = self.inNum * self.tranNum
        if input.ndim == 4 and input.shape[1] == expected and input.shape[-1] != expected:
            return self.forward_nchw(input)
        if input.shape[-1] != expected:
            if input.ndim == 4 and input.shape[1] == expected:
                return self.forward_nchw(input)
            raise ValueError(
                f"expected packed dimension {expected}, got shape {tuple(input.shape)}"
            )
        return self.forward_last_dim(input)

    def forward_last_dim(self, input):
        weight, bias = self._weight_bias()
        return F.linear(input, weight, bias=bias)

    def forward_nchw(self, input):
        """Apply the cached dense EQ map with a last-axis Linear kernel."""
        weight, bias = self._weight_bias()
        output = F.linear(input.permute(0, 2, 3, 1), weight, bias=bias)
...
class EQ_linear_output(nn.Module):
    ...
    def forward(self, input):
        expected = self.inNum * self.tranNum
        if input.shape[-1] != expected:
            raise ValueError(f"expected last dim {expected}, got {input.shape[-1]}")
        pooled = input.reshape(
            *input.shape[:-1], self.inNum, self.tranNum
        ).sum(dim=-1)
        return F.linear(pooled, self.weights.squeeze(-1), bias=self.c)
...
class EQDropout(nn.Module):
    """Equivariant dropout for either NCHW or NHWC block layouts."""

    def __init__(self, p=0.5, tranNum=4, channel_first=True):
        super().__init__()
        self.p = p
        self.tranNum = tranNum
    ...
    def forward(self, x):
        ...
        output = grouped.reshape(batch, packed_channels, height, width)
        if not self.channel_first:
...
class EQLayerNorm(EQLayerNorm2d):
    """C4-shared LayerNorm for a packed NHWC activation."""
...
class EQSS2D(nn.Module):
    ...
    def __init__(self, d_model=96, d_state=1, ssm_ratio=1.0, tranNum=4, dt_rank="auto", act_layer=nn.SiLU, d_conv=3,
        ...
        self.eq_in_proj = EQ_linear_inter(self.d_model // tranNum, d_proj // tranNum, tranNum=tranNum, bias=bias)
        self.act = act_layer()
        if self.with_dconv:
    ...
    def forward(self, x, **kwargs):
        ...
        if not self.channel_first:
            x = x.permute(0, 2, 3, 1)
        return self.dropout(self.eq_out_proj(x))
...
class EQVSSBlock(nn.Module):
    def __init__(
        ...
        self.drop_path = EQDropout(
            drop_path, tranNum=tranNum, channel_first=channel_first
        )  # debug， 20251012
...
class EQVSSM_V2(nn.Module):
    def __init__(
            ...
            drop_path_rate=0.1,
            patch_norm=True,
            norm_layer="ln",  # NHWC EQ LayerNorm; use "ln2d" for NCHW
        ...
        norm_layer: nn.Module = EQLayerNorm2d if self.channel_first else EQLayerNorm
...
class Backbone_EQVSSM_Joint(EQVSSM_V2):
    def __init__(self, out_indices=(0, 1, 2, 3), eq_tranNum=4, pretrained=None, norm_layer="ln", **kwargs):
        ...
        for i in out_indices:
            norm_cls = EQLayerNorm2d if self.channel_first else EQLayerNorm
            layer = norm_cls(self.dims[i] // eq_tranNum)
...
if __name__ == "__main__":
    ...
    run_throughput_cli(
        ...
        target_classes=(EQ_linear_inter, EQ_linear_output),
        tran_num=4,
