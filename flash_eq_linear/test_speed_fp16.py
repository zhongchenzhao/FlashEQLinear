#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Compare FP16 dense, naive EQLinear, PyTorch FlashEQLinear, and CUDA paths."""
...
from kernels.flash_eqlinear_fp16.flash_EQLinear_fp16_direct_gemm_release_ada4090 import FlashEQLinearFp16DirectGemmAda4090
from flash_eq_linear.flash_eq_linear_torch import Flash_EQ_linear_Torch


DTYPE = torch.float16
FP16_BACKEND = "pure_cute_fused"


def make_cuda_eqlinear_fp16(C_in, C_out, *, eqlinear_weights=None, tranNum=4, bias=False, eqlinear_bias=None):
    """Create FP16 Flash EQ-Linear layer while tolerating minor constructor differences.

    Some local versions expose only the release constructor, while routed/auto versions
    may accept backend/fallback_backend. This helper keeps the speed script robust.
    """
    common_kwargs = dict(
        eqlinear_weights=eqlinear_weights,
        tranNum=tranNum,
        bias=bias,
        eqlinear_bias=eqlinear_bias,
    )
    try:
        return FlashEQLinearFp16DirectGemmAda4090(
            C_in, C_out, **common_kwargs, fallback_backend=FP16_BACKEND
        )
    except TypeError:
        try:
            return FlashEQLinearFp16DirectGemmAda4090(
                C_in, C_out, **common_kwargs, backend=FP16_BACKEND
            )
        except TypeError:
            return FlashEQLinearFp16DirectGemmAda4090(C_in, C_out, **common_kwargs)
...
def _cuda_time_backward(layer, x, grad_y, iters, warmup=5):
    """Return backward ms/iter, resetting gradients on every iteration."""
    y = layer(x)
...
def test_precision(device):
    print("\n" + "=" * 50)
    print("1. FP16 Precision Test (Forward & backward)".center(50))
    print("=" * 50)

    B, L, C_in, C_out, T = 2, 56 * 56, 64, 64, 4

    x_naive = torch.randn(B, L, C_in * T, device=device, dtype=DTYPE, requires_grad=True)
    x_torch = x_naive.detach().reshape(B, L, C_in, T).clone().requires_grad_(True)
    x_cuda = x_naive.clone().detach().requires_grad_(True)

    naive_model = EQ_linear_inter(C_in, C_out, tranNum=T, bias=True).to(device=device, dtype=DTYPE)
    ...
    torch_model = Flash_EQ_linear_Torch(
        C_in, C_out, eqlinear_weight=spatial_weight, tranNum=T, bias=True,
        device=device
    ).to(device=device, dtype=DTYPE)
    cuda_model = FlashEQLinearFp16DirectGemmAda4090(
        C_in, C_out, eqlinear_weights=None, tranNum=T, bias=True
    ).to(device=device, dtype=DTYPE)
    with torch.no_grad():
        torch_model.c.copy_(bias)
        cuda_model.weights.copy_(torch_model.W)
        cuda_model.c.copy_(torch_model.c)

    print(f"torch_model weight dtype: {torch_model.W.dtype}")
    print(f"cuda_model weight dtype: {cuda_model.weights.dtype}")
    if hasattr(cuda_model, "c") and cuda_model.c is not None:
    ...
    # Map gradients in FP32 to avoid additional rounding during comparison.
    torch_spatial_grad = freq_grad_to_spatial(torch_model.W.grad.float())
...
def benchmark(device):
    ...
    print("=" * len(header))
    print("2. FP16 Speed Test (Forward & Backward)".center(len(header)))
    print("=" * len(header))
    print("Torch method: Flash_EQ_linear_Torch (FP16)")
    ...
    for B in [32]:
        for L in [128, 256, 512, 1024, 4096]:
            for C_in in [16, 32, 64, 128, 256, 512, 1024, 2048]:
                ...
                noneq_linear = Standard_linear_inter(
                    C_in, C_out, tranNum=T, bias=False
                ).to(device=device, dtype=DTYPE)
                ...
                torch_layer = Flash_EQ_linear_Torch(
                    C_in, C_out, eqlinear_weight=spatial_weight, tranNum=T, bias=False,
                    device=device
                ).to(device=device, dtype=DTYPE)

                cuda_layer = FlashEQLinearFp16DirectGemmAda4090(
                    C_in, C_out,
                    eqlinear_weights=None,
                    tranNum=T,
                    bias=False
                ).to(device=device, dtype=DTYPE)

                # Match frequency weights before warmup and weight packing.
                with torch.no_grad():
                    cuda_layer.weights.copy_(torch_layer.W)
                ...
                # Keep one graph at a time and reset gradients between backward iterations.
