...
from kernels.flash_eqlinear_fp16.flash_EQLinear_fp16_direct_gemm_release_ada4090 import (
    FlashEQLinearFp16DirectGemmAda4090 as Cuda_Flash_EQ_linear,
)
from kernels.flash_eqlinear_fp16.flash_EQLinear_fp16_direct_gemm_pipeline_ada4090 import (
...
# 3. 辅助函数
# ==========================================
def print_error(name, x_test, x_ref, epsilon=1e-12):
...
def print_paper_metrics_table(metrics_dict):
    ...
    """
    print("\n" + "=" * 120)
    print("Flash EQ-Linear Precision Errors (fp16)".center(120))
    print("=" * 120)
...
def test_precision(device):
    ...
    x_naive = torch.randn(B, L, C_in * T, device=device, dtype=torch.float16, requires_grad=True)
    x_cuda = x_naive.clone().detach().requires_grad_(True)

    naive_model = EQ_linear_inter(C_in, C_out, tranNum=T, bias=True).to(device=device, dtype=torch.float16)
    spatial_weight = naive_model.weights[:, 0].detach().contiguous().half()
    bias = naive_model.c.detach().contiguous().half()
    ...
    metrics["Y"].update({"Tensor": "Forward output Y", "Dtype": "FP16", "Shape": str(tuple(y_cuda.shape))})
    metrics["dX"].update({"Tensor": "Backward input grad dX", "Dtype": "FP16", "Shape": str(tuple(x_cuda.grad.shape))})
    metrics["dW"].update({"Tensor": "Backward weight grad dW", "Dtype": "FP16", "Shape": str(tuple(cuda_spatial_grad.shape))})
    metrics["db"].update({"Tensor": "Backward Bias Grad db", "Dtype": "FP16", "Shape": str(tuple(cuda_model.c.grad.shape))})
    metrics["db_expected"].update({"Tensor": "db vs expected sum", "Dtype": "FP16", "Shape": str(tuple(expected_db.shape))})
    print_paper_metrics_table(metrics)




def torch_cuda_math_forward(x, w_freq, bias=None, return_dtype=None):
    """
    x:      [B, L, I, 4], FP16/FP32
    w_freq: [O, I, 4],   FP16/FP32
    bias:   None or [O, 1]/[O]. Same semantics as EQLinearInter.c:
            one scalar per output channel, repeated across T=4.
    return: [B, L, O, 4]

    For the FP16 precision test, this function uses FP32 arithmetic on FP16-rounded
    inputs/weights, then casts the output back to return_dtype. This is useful as a
    stable PyTorch reference for TensorCore/FP16 CUDA kernels with FP32 accumulation.
    """
    ...
    return y.to(dtype=return_dtype)
...
def diagnose_cuda_vs_simulated_paper_metrics(
    ...
    # 1. Prepare inputs
    # -------------------------
    x0 = torch.randn(B, L, C_in, T, device=device, dtype=torch.float16)

    spatial_w = torch.randn(
        C_out, C_in, T, device=device, dtype=torch.float16
    ) * 0.05

    w_freq0 = spatial_to_freq_weight(spatial_w).contiguous()
    bias0 = (torch.randn(C_out, 1, device=device, dtype=torch.float16) * 0.05).contiguous()

    grad_y0 = torch.randn(B, L, C_out, T, device=device, dtype=torch.float16)

    # -------------------------
    # 2. PyTorch simulated CUDA math reference
    # -------------------------
    x_ref = x0.detach().clone().half().requires_grad_(True)

    # Important:
    # real CUDA uses weight, so reference should use rounded weight too.
    w_ref = w_freq0.detach().clone().half().requires_grad_(True)
    b_ref = bias0.detach().clone().half().requires_grad_(True)

    gy_ref = grad_y0.detach().clone().half()

    y_ref_fp32 = torch_cuda_math_forward(x_ref, w_ref, b_ref, return_dtype=torch.float16)
    ...
    db_expected = grad_y0.float().sum(dim=(0, 1, 3)).view(C_out, 1).half()
    ...
    y_cuda = FlashEQLinearDirectGemmFp16RoutedFunction.apply(x_cuda, w_cuda, b_cuda, "pure_cute_fused")
...
def exact_small_test(device):
    ...
    grad_y = torch.ones(B, L, C_out, T, device=device, dtype=torch.float16)
    ...
    y_cuda = FlashEQLinearDirectGemmFp16RoutedFunction.apply(x_cuda, w_cuda, None, "pure_cute_fused")
    y_cuda.backward(grad_y)
...
def repeat_stability_test(device, repeats=20):
    ...
    x = torch.randn(B, L, C_in, T, device=device, dtype=torch.float16)
    w = torch.randn(C_out, C_in, T, device=device, dtype=torch.float16)
    gy = torch.randn(B, L, C_out, T, device=device, dtype=torch.float16)
    ...
    for _ in range(repeats):
        ...
        y = FlashEQLinearDirectGemmFp16RoutedFunction.apply(x1, w1, None, "pure_cute_fused")
        y.backward(gy)
